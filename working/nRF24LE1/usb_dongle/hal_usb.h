...
/** @file
 * @brief Header file defining the descriptors used in an application.
 *
 ...
 *  - g_usb_dev_desc
 *
 * @defgroup hal_nrf24lu1p_hal_usb hal_usb
 * @{
 * @ingroup hal_nrf24lu1p
 * This module defines an interface towards a USB device controller.
 *
 ...
 *
 * The module do <b>not</b> support SetDescriptor requests
 *
...
typedef struct {
    uint8_t  bmRequestType; /**< Bitmapped field identifying the characteristics of the request.
                        ...
                        - 4..31 = Reserved
                          */
    uint8_t  bRequest;       /**< Field specifying request. bmRequestType(Type) modify the meaning of this field. */
    ...
    uint8_t wIndex;         /**< Field used to pass a paramters to the device, specific to the request. */
...
/** An enum describing what reply to send to the control request
 */
...
/** Callback function that is called when an endpoint interrupt occur 
 ...
 */
typedef uint8_t (*hal_usb_cb_endpoint_t)(uint8_t* adr_ptr, uint8_t* size) reentrant;
...
/** Callback function that is called when a suspend condition occur.
...
/** Callback function that is called when a reset condition occur.
 *  
 */
...
typedef struct {
    ...
    uint8_t current_alt_interface; /**< Currently alternative configuration. If a alternative configuration is chosen the index of the alternative configuration is stored here. */
 ...
 *  @param usb_disconnect Set to true to perform physical disconnect and reconnect. Make sure to enable interruts as soon as possible after
 *                        the call to this function.
 *  @param device_req Pointer to function to call when a class specific request occur
 *  @param reset Pointer to function to call when USB controller detect a reset
 *  @param resume Pointer to function to call when USB controller detect a resume
 *  @param suspend Pointer to function to call when USB controller detect a suspend
 */
void hal_usb_init(
    bool usb_disconnect,
    hal_usb_cb_device_req_t device_req,
