...
#define RELOAD_COUNT  0xfd   //11080373Hz,12T,SMOD=0,9600bps
...
void main(void)
{
    ...
    serial_port_one_initial();      //??1???
//    serial_port_two_initial();      //??2???
    display_MCU_Start_Led();    //????????????????

 //   send_UART_two(0x55);            //??2???????????????
  //  send_UART_two(0xaa);            //??2???????????????
/*
...
*/
    send_UART_one(0x34);            //??1???????????????
    send_UART_one(0xa7);            //??1???????????????


//    for(array_point=0; array_point<512; array_point++)

    for(array_point=0; array_point<8; array_point++)
    {
		send_UART_one(Test_array_one[array_point]+'0');
    }
